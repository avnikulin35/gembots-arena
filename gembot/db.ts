...
// Migrations
try { db.exec(`ALTER TABLE files ADD COLUMN data BLOB`); } catch {}
try { db.exec(`ALTER TABLE files ADD COLUMN text_content TEXT`); } catch {}
...
try { db.exec(`ALTER TABLE reminders ADD COLUMN notified INTEGER DEFAULT 0`); } catch {}
// Add notes column to reminders
try { db.exec(`ALTER TABLE reminders ADD COLUMN notes TEXT`); } catch {}
// Add updated_at column to reminders
try { db.exec(`ALTER TABLE reminders ADD COLUMN updated_at TEXT DEFAULT (datetime('now'))`); } catch {}
...
// Family access
try { db.exec(`CREATE TABLE IF NOT EXISTS families (
  id TEXT PRIMARY KEY,
  name TEXT,
...
try { db.exec(`CREATE TABLE IF NOT EXISTS family_memories (
  id TEXT PRIMARY KEY,
  family_id TEXT,
  fact TEXT,
  category TEXT,
...
try { db.exec(`CREATE INDEX IF NOT EXISTS idx_family_memories_family ON family_memories(family_id)`); } catch {}
try { db.exec(`ALTER TABLE users ADD COLUMN family_id TEXT`); } catch {}
try { db.exec(`ALTER TABLE users ADD COLUMN family_role TEXT DEFAULT NULL`); } catch {}
try { db.exec(`ALTER TABLE users ADD COLUMN family_invite_code TEXT`); } catch {}

// Add language column to user_settings (legacy DBs)
try { db.exec(`ALTER TABLE user_settings ADD COLUMN language TEXT DEFAULT 'auto'`); } catch {}
// Normalize legacy default language to new auto-detect mode
try { db.exec(`UPDATE user_settings SET language = 'auto' WHERE language IS NULL OR language = '' OR language = 'ru'`); } catch {}
