...
async function main() {
  ...
  await test("10. PM2 vitalik-bot", async () => {
    ...
    const bot = list.find((p: any) => p.name === "vitalik-bot");
    ok("10. PM2 vitalik-bot online", bot?.pm2_env?.status === "online", `status: ${bot?.pm2_env?.status}`);
  });

  console.log(`\n📊 Results: ${passed} passed, ${failed} failed`);
  if (failed === 0) console.log("🎉 All tests passed! Bot MVP is ready.");
  else console.log("⚠️ Some tests failed - needs attention.");
}
