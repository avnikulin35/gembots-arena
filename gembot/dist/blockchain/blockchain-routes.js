...
router.get('/wallet', (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const row = db.prepare('SELECT wallet_address FROM users WHERE id = ?').get(userId);
...
router.post('/record-memory', async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const existing = db.prepare('SELECT * FROM memory_hashes WHERE memory_hash = ?').get(memoryHash);
        if (existing) {
            return res.json({
                success: true,
...
router.get('/verify-memory', (req, res) => {
    try {
        const hash = req.query.hash;
        if (!hash)
            return res.status(400).json({ error: 'hash query param required' });
        const record = db.prepare('SELECT * FROM memory_hashes WHERE memory_hash = ?').get(hash);
        if (!record) {
