...
// ==================== CHAINGPT ====================
async function callChainGPTFromBot(messages) {
    ...
    if (!res.ok)
        throw new Error(`ChainGPT API ${res.status}`);
    const data = (await res.json());
    return data.data?.bot || "";
}
// ==================== SYSTEM PROMPT ====================
function buildSystemPrompt(userId, skill) {
...
async function perplexitySearch(query) {
    ...
    const contents = await Promise.all(results.slice(0, 3).map(async (r, i) => {
...
async function extractFacts(userId, userMsg, assistantMsg) {
    try {
        ...
        for (const f of facts) {
            if (!f.fact || f.fact.length < 3 || dbHelpers.getMemoryCount(userId) >= 50)
                continue;
    ...
    return new Promise(async (resolve) => {
        ...
        try {
            ...
            ffmpeg.on("close", (code) => {
                ...
                if (code === 0 && fs.existsSync(tmpOgg))
                    resolve(tmpOgg);
                else {
                    try {
...
async function ytSubtitles(url) {
...
// ==================== HELPERS ====================
function uid(msg) { return `tg_${msg.from.id}`; }
...
function withBotErrorBoundary(label, handler) {
    return async (...args) => {
        try {
            await handler(...args);
        }
...
// ==================== MAIN CHAT ====================
async function handleText(msg, text, isVoice = false) {
    ...
    try {
        ...
        let urlContent = null;
        if (urlMatch && !ytUrl)
            urlContent = await fetchPage(urlMatch[0]);
        // Search
        let searchContent = "";
        let searchSources = [];
        ...
        if (isVoice) {
            ...
            if (audioPath) {
                try {
                    ...
                    await bot.sendVoice(msg.chat.id, fs.createReadStream(audioPath), {}, { filename: "voice.ogg", contentType: "audio/ogg" });
                }
                catch (voiceErr) {
...
bot.onText(/\/help/, withBotErrorBoundary("/help", async (msg) => {
    await bot.sendMessage(msg.chat.id, `💎 <b>GemBot Commands</b>\n\n/start — Welcome\n/search — Web search\n/skill — Switch crypto persona\n/memory — Your memories\n/credits — Balance\n/bonus — Daily bonus\n/buy — Buy credits ⭐\n/help — Help\n\nJust type, send a photo, voice or PDF!`, { parse_mode: "HTML" });
}));
...
function markStarsPaymentPaid(params) {
    const tx = db.transaction((input) => {
        const existing = db.prepare(`SELECT * FROM billing_payments WHERE invoice_id = ? LIMIT 1`).get(input.invoiceId);
        if (!existing) {
            db.prepare(`
      ...
      `).run(input.invoiceId, input.userId, input.packageId, input.credits, input.stars, input.invoicePayload, input.telegramPaymentChargeId || null, input.providerPaymentChargeId || null);
        }
        else {
            db.prepare(`
      ...
      `).run(input.telegramPaymentChargeId || null, input.providerPaymentChargeId || null, input.invoiceId);
        ...
        const payment = db.prepare(`SELECT * FROM billing_payments WHERE invoice_id = ? LIMIT 1`).get(input.invoiceId);
        if (!payment.credited) {
...
bot.on('message', withBotErrorBoundary('payment_message', async (msg) => {
